...
-Классы Unit и UnitSkill
-Локальное хранилище способностей ScriptableObject
-Локальное хранилище персонажей ScriptableObject
-Получение информации о способностях и перонажах из БД сервера с помощью REST API
-Локатор сервисов ServiceLocator
-Скрипты персонажа
-Интерфейсы
-Тест
...
Далее создадим надстройку GET_UnitSkills, которая будет формировать конкретный запрос для скиллов, получать респонс с сервера, а затем десериализировать его с помощью JsonConvert:
...
Cоздаем скрипты, которые будут хранить ссылки на инструменты работы с веб-реквестами и локальной БД с персонажами и их способностями:
...
На старте мы получаем скиллы с помощью ServiceLocator:

-либо из базы данных сервера через запрос из WebRequestUnit

-либо через локальную базу данных через запрос из UnitLocalSource
